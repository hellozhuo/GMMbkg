...
cv::Mat getBG(const cv::Mat& srcorin, double reg, double ratio)
{
	assert(srcorin.channels() == 3);
	cv::Mat src;
	srcorin.convertTo(src, CV_32FC3);
	double h = (double)(src.rows);
	double w = (double)(src.cols);
	int rowMargin = std::round(ratio*h);
	int colMargin = std::round(ratio*w);
	std::vector<cv::Mat> roi, cov, mmean;
	cv::Mat uproi = src(cv::Rect(0, 0, src.cols, rowMargin)),upcov,upmean;
	roi.push_back(uproi.clone()); cov.push_back(upcov); mmean.push_back(upmean);
	cv::Mat downroi = src(cv::Rect(0, src.rows - rowMargin, src.cols, rowMargin)),downcov,downmean;
	roi.push_back(downroi.clone()); cov.push_back(downcov); mmean.push_back(downmean);
	cv::Mat leftroi = src(cv::Rect(0, 0, colMargin, src.rows)),leftcov,leftmean;
	roi.push_back(leftroi.clone()); cov.push_back(leftcov); mmean.push_back(leftmean);
	cv::Mat rightroi = src(cv::Rect(src.cols - colMargin, 0, colMargin, src.rows)),rightcov,rightmean;
	roi.push_back(rightroi.clone()); cov.push_back(rightcov); mmean.push_back(rightmean);
	cv::Mat src_vec((src.cols)*(src.rows), 3, CV_32FC1);
	src_vec.data = src.data;
	int num = roi.size();
	std::array<cv::Mat,4> dis;
	for (int i = 0; i < num; i++)
	{
		cv::Mat s((roi[i].rows)*(roi[i].cols), 3, CV_32FC1);
		s.data = roi[i].data;
		cv::calcCovarMatrix(s, cov[i], mmean[i], CV_COVAR_NORMAL | CV_COVAR_ROWS,CV_32F);
		cov[i].convertTo(cov[i], cov[i].type(), 1.0 / (s.rows - 1.0));
		cov[i] += cv::Mat::eye(cov[i].size(), CV_32FC1)*reg;
		Mat w, u, vt;
		cv::SVD::compute(cov[i], w, u, vt);
		cv::sqrt(w, w);
		cv::Mat white = u * cv::Mat::diag(1.0 / w);
		cv::repeat(mmean[i], src_vec.rows, 1, mmean[i]);
		cv::Mat tmp = cv::abs((src_vec - mmean[i])*white);
		dis[i] = tmp(cv::Rect(0, 0, 1, tmp.rows)) + tmp(cv::Rect(1, 0, 1, tmp.rows))
			+ tmp(cv::Rect(2, 0, 1, tmp.rows));
		cv::normalize(dis[i], dis[i], 0.0, 1.0, cv::NORM_MINMAX);

	}
	cv::Mat ret = matMax(dis[0], dis[1], dis[2], dis[3]);
	ret = dis[0] + dis[1] + dis[2] + dis[3] - ret;
	normalize(ret, ret, 0.0, 1.0, cv::NORM_MINMAX);
	cv::Mat ret_normal(src.rows, src.cols, CV_32FC1);
	ret_normal.data = ret.data;
	return ret_normal.clone();
}

cv::Mat matMax(const cv::Mat& src1, const cv::Mat& src2, const cv::Mat& src3, const cv::Mat& src4)
{
	cv::Mat res;
	cv::max(src1, src2, res);
	cv::max(src3, res, res);
	cv::max(src4, res, res);
	return res;
}
...
void morphSmooth(cv::Mat& dMap, int radius)
{
	assert(dMap.type() == CV_32FC1);
	cv::Mat img;
	...
	cv::normalize(Iobrcbr, Iobrcbr, 0.0, 1.0, cv::NORM_MINMAX);
	Iobrcbr.copyTo(dMap);
	return;	 
}
